...
class KillInterruptionHandler(AbstractInterruptionHandler):
    ...
    def execute(self, irq):
        ...
        self.kernel.memoryManager.freeFrames(pcbKill.getBaseDir()) #borra la pag pasada por parametro, no nos importa el tamaño porque son todas del mismo tamaño   
...
class NewInterruptionHandler(AbstractInterruptionHandler):
    ...
    def execute(self, irq):
        ...
        baseDir        = self.kernel.loader.load_program(path) #ahora es una page table
...
class Loader ():
    ...
    def load_program(self, path):
        #hacer page table
        programIns = self.kernel.fileSystem.read(path).instructions
        pageTable = {}
        frameSize = self.kernel.memoryManager.frameSize()
        cantPag = math.ceil(len(programIns) / frameSize) #cantidad de paginas en que se dividen las instrucciones de un programa (division entera cantidad de instrucciones / frameSize)
       #si hay frames disponibles para la cantidad de paginas requerida
        if len(self.kernel.memoryManager.getFreeFrame()) >= cantPag:
            #hacer page table
            framesDeLasPag = self.kernel.memoryManager.allocFrames(cantPag) #lista de frames dados para llenar
            paginas = self.paginas(cantPag, frameSize, programIns)
            numeroPaginaActual = 0
            for frame in framesDeLasPag:
                pageTable[numeroPaginaActual] = frame
                self.cargarMemoria(paginas.pop(0), frame, frameSize) #cargo la pagina actual en el frame actual
    ...
    def cargarMemoria(self, pag, frame, frameSize): #la pagina tiene instrucciones dentro
        for i in range(len(pag)):
            HARDWARE.memory.write((frame * frameSize) + i, pag[i])

    #lista de paginas que necesito segun la cantidad de instrucciones del programa dado
    def paginas(self, cantPag, frameSize, program):
        paginas = []
        for _ in range(cantPag):
            paginas.append(self.pagina(frameSize, program))
        return paginas
    
    #devuelve una pagina que tiene unalista de instrucciones
    def pagina(self, frameSize, program):
        pagina = []
        for _ in range(frameSize):
            if len(program) > 0: #si el programa todavia tiene instrucciones lo pone
                pagina.append(program.pop(0))
        return pagina
...
class  Dispatcher():
    ...
    def load(self, pcb):
        ...
        #tlb es copia de la page table (funciona como cache)
        tbl = pcb.getBaseDir() #page table del pcb
         ## al hacer un context switch
        HARDWARE.mmu.resetTLB()
        for i in tbl.keys(): #por cada clave del diccionario
            HARDWARE.mmu.setPageFrame(i, tbl[i]) #creo la tabla, con esa pagina y su valor
...
class MemoryManager():
    def __init__(self, kernel): 
        ...
        self._freeMem = HARDWARE.memory.size
        self._freeFrames = [] #arranca con todos los frames disponibles desde 0
        self.calcFrameMemory(self._freeMem, self._frameSize) # para que el freeFrames arranque con todos los frames libre y no como una lista vacia, ya que al principio la memoria esta vacia

    def calcFrameMemory(self, mem, frameMem):
        for i in range(int(mem / frameMem)):
            self._freeFrames.append(i)

# te devuelve los frames que vas a usar segun las paginas que tengas
    def allocFrames(self, int):
        framePut = []
        for i in range(int):
            framePut.append(self._freeFrames.pop(0))
            self._freeMem -= self._frameSize
        return framePut

    def freeFrames(self, pageTable): #libera los frames de la page table pasada por parametro y los agrega a la lista 
        for i in pageTable.keys():
            self._freeFrames.append(pageTable[i]) #le agrego a free frames, el frame que corresponde a esa pagina (i)
            self._freeMem += self._frameSize
...
class FileSystem():
    def __init__(self):
        self.almacenamientoDisco = {} #un diccionario que sea clave (el path) valor (el programa)
                                    #los path son únicos

    def write(self, path, program):
        self.almacenamientoDisco[path] = program #le asignamos a ese path, ese programa

    def read(self, path):
        return self.almacenamientoDisco.get(path) #obtenemos el valor(el programa) de esa clave que es el path (direccion)
...
class Kernel():
    ...
    def __init__(self):
        ...
        self._ioDeviceController = IoDeviceController(HARDWARE.ioDevice)
         ...
         #Boot del S.O. : Tenemos que setearle el frameSize al MM
   ...
   # @property
    ...
    def run(self, path, priority): #ahora le llega la prioridad para los scheduler de eso
